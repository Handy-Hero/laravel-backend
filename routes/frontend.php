...
Route::middleware([CheckInstallation::class])->group(function () {
    Route::get('/landing', [FrontendController::class, 'index'])->name('frontend.index');
    Route::get('/', [FrontendController::class, 'userLoginView'])->name('user.login');
    Route::get('/', [HomeController::class, 'authLogin'])->name('auth.login');
    Route::get('/admin', [HomeController::class, 'authLogin'])->name('auth.login');
